...
app.get('/', async (request, response) => {
  response.writeHead(301, { Location: 'https://github.com/RishikeshOps/Hashnode-' });
  response.end();
})
...
app.get('/getLatestHashnodeBlog', async (request, response) => {
  try {
    ...
    const large = request.query.large === "true" ? true : false
    const resultData = (await getLatestHashnodeBlog(RushikeshOps));
...
app.get('/getHashnodeBlogBySequence', async (request, response) => {
  try {
    ...
    const resultData = (await getLatestHashnodeBlog(RushikeshOps, page));
